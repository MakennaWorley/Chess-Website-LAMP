$(document).ready(function () {
    ...
    //------------------------------------------------------------------------------input1 query----------------------//
    // See code in query.php for source
    window.queryClass = function (entry) { //this is from a buttton
        $.ajax({
            type: "POST",
            url: "query.php", // Replace with the actual path to your PHP file
            data: {
    ...
    window.getUpdatePlayerNameClass = function () {
        var userInputValue = document.getElementById("userInput").value;
        if (userInputValue.trim() === "") {
            console.log("Invalid entry");
            $("#result").html("Please try again");
    ...
    window.getDeleteGame = function () {
        ...
        else if (/^[1-9]?[1-9]$/.test(boardNumber) == false) {
            console.log("Invalid boardNumber");
            $("#result").html("Must be a number between 1-99");
        }
        $.ajax({
...
function queryClassForm2(className, playerId) {
    $.ajax({
        type: "POST",
        url: "query.php",
        data: {
        ...
        success: function (response) {
            console.log("Keyword is:" + className);
            $("#resultTable").html(response);
        },
        error: function (error) {
            console.error("Error:", error);
...
function sendResult(result, letter, number) { //this uses buttons
    $.ajax({
        type: "POST",
        url: "query.php",  // Replace with the actual server-side script URL
        data: {
